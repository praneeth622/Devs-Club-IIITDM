    ...
    const EventCard = ({ event, isPastEvent, onViewDetails }) => (
      <motion.div
        ...
        className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-lg overflow-hidden 
                  ...
                  flex flex-col h-[350px] border-0 dark:text-white/80 dark:bg-gray-900"
      >
        {isPastEvent && (
        ...
        <div className="p-6 flex flex-col flex-grow">
          <div className="mb-3 flex justify-between items-start">
            <h3 className="text-xl font-bold text-gray-900 hover:text-blue-600 transition-colors dark:text-white/80">{event.Event_name}</h3>
          </div>
          <p className="text-gray-600 mb-4 flex-grow line-clamp-3 dark:text-white/40">{event.Event_details}</p>
          <div className="space-y-2 border-t border-gray-100 pt-4">
            <div className="flex items-center text-gray-600 dark:text-white/40">
          ...
          >
            ...
            <div className="text-center mb-20">
              <motion.div
                ...
                className="inline-block mb-4 px-6 py-2 bg-blue-100 text-blue-600 rounded-full text-sm font-semibold tracking-wide dark:bg-gray-800"
              >
                EVENTS & WORKSHOPS
              </motion.div>
              <motion.h1
            ...
            <div className="flex justify-center mb-12">
              <div className="inline-flex rounded-xl shadow-sm p-1 bg-white dark:text-white/80 dark:bg-gray-800" role="group">
                <button
                  onClick={() => setActiveTab('upcoming')}
                  className={`px-6 py-3 text-sm font-medium rounded-lg transition-all duration-200  ${
                    activeTab === 'upcoming'
                      ? 'bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl text-white shadow-lg'
                      : 'bg-transparent text-gray-700 hover:bg-gray-50 rounded-xl dark:hover:bg-gray-900'
                  }`}
                >
                  Upcoming Events
                </button>
                <button
                  ...
                  className={`px-6 py-3 text-sm font-medium rounded-lg transition-all duration-200 dark:bg-gray-800 ${
                    activeTab === 'past'
                      ? 'bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl text-white shadow-lg'
                      : 'bg-transparent text-gray-700 hover:bg-gray-50 rounded-xl dark:hover:bg-gray-900'
                  }`}
            ...
            {!isLoading && (activeTab === 'upcoming' ? upcomingEvents : pastEvents).length === 0 && (
              ...
              >
                <div className="bg-white rounded-2xl p-8 shadow-lg max-w-md mx-auto dark:text-white/80 dark:bg-gray-900">
                  <div className="text-6xl mb-4">🎉</div>
                  <h3 className="text-xl font-semibold text-gray-900 mb-2 dark:text-white/80">No {activeTab} events at the moment</h3>
                  <p className="text-gray-600 dark:text-white/40">Check back soon for more exciting events!</p>
                </div>
              </motion.div>
            )}
          </motion.div>
                ...
                <motion.h2
                  ...
                  >
                    <Link
                      ...
                      className="inline-flex items-center justify-center px-6 sm:px-10 py-3 sm:py-4 bg-white text-blue-800 rounded-full font-semibold hover:bg-blue-50 transition-all duration-300 shadow-xl hover:shadow-2xl transform hover:-translate-y-1 text-sm sm:text-lg gap-2 dark:bg-gray-800"
                    >
                      Join Now
                    </Link>
                  </motion.div>
